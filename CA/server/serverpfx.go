...
func main() {
	c, err := ioutil.ReadFile("./server.pfx")
	if err != nil {
		fmt.Printf("ioutil.ReadFile err: %s\n", err.Error())
		return
	}
	/*p12, cert, err := pkcs12.Decode(c, "golua")
	if err != nil {
		fmt.Printf("pkcs12.Decode err: %s\n", err.Error())
		return
	}
	fmt.Printf("p12 is %T, cert is %v\n", p12, cert)
	return*/

	blocks, err := pkcs12.ToPEM(c, "golua1")
	if err != nil {
		panic(err)
	}
	...
	if !ok {
		fmt.Printf("clientCertPool.AppendCertsFromPEM err: %s\n", err.Error())
		return
	}
	...
	for {
		...
		i ++
		fmt.Printf("i is %d\n", i)
		go handlerConnP(conn)
	}
}

func handlerConnP(conn net.Conn) {
	defer conn.Close()
	r := bufio.NewReader(conn)
	for {
		n, err := conn.Write([]byte("world\n"))
		if err != nil {
			log.Println("server conn", n, err)
			return
		}
