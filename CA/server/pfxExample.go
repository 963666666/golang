...
func main() {
	...
	p12, _ := base64.StdEncoding.DecodeString(testdata["testing@example.com"])

	blocks, err := pkcs12.ToPEM(p12, "111")
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		go handlerConnPfx(conn)
	}
}


func handlerConnPfx(conn net.Conn) {
	defer conn.Close()
